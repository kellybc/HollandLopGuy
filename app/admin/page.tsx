...
export default function AdminPage() {
  ...
  const [newFaculty, setNewFaculty] = useState<{ name: string; program: Program; rank_or_role: string; annual_workload_target: number; fall_target: number; winter_target: number; spring_target: number; summer_target: number }>({ name: '', program: 'Mechanical Engineering', rank_or_role: 'Faculty', annual_workload_target: 18, fall_target: 6, winter_target: 6, spring_target: 6, summer_target: 0 });
    ...
    <div className="space-y-4">
      ...
      <section className="rounded-lg border bg-white p-4">
        ...
        <div className="mb-3 grid gap-2 md:grid-cols-4">
          ...
          <button onClick={createFaculty} className="rounded bg-slate-900 px-3 py-2 text-sm text-white">Add Faculty</button>
        </div>
      ...
      <section className="rounded-lg border bg-white p-4">
        ...
        <div className="mb-3 grid gap-2 md:grid-cols-7">
          <select className="rounded border p-2 text-sm" value={newAssignment.faculty_id} onChange={(e) => setNewAssignment((p) => ({ ...p, faculty_id: e.target.value }))}>{faculty.map((f) => <option key={f.id} value={f.id}>{f.name}</option>)}</select>
            ...
            <tbody>
              {assignments.map((a) => (
                <tr key={a.id}>
                  ...
                  <td className="border p-2"><select className="rounded border p-1" value={a.faculty_id} onChange={(e) => setAssignments((prev) => prev.map((row) => row.id === a.id ? { ...row, faculty_id: e.target.value } : row))}>{faculty.map((f) => <option key={f.id} value={f.id}>{f.name}</option>)}</select></td>
